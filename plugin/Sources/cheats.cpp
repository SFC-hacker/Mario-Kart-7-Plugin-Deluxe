...
namespace CTRPluginFramework
{
	...
	static bool IsNew3DS = false;
	
	static bool IsCitra = false;
	...
	void	writeRoomID(u32 roomID)// Unused
	{
		if (Process::Read32(0x663D04, offset))
		{
			Process::Read32(offset + 0x50, offset);
			Process::Read32(offset + 0x44, offset);
			Process::Write32(offset + 0x40, roomID);
		}
	}
	...
	{	
		...
		{
			entry->Name() = "Blinking Invincibility";
	...
	{	
		if(entry->WasJustActivated())
		{
			entry->Name() = Color::LimeGreen << "Always Have Star Power";
		}
		...
		if (!entry->IsActivated())
		{
			entry->Name() = "Always Have Star Power";
	...
	//max speed forwoard: 665410 = min I searched to
	...
	void	dropMushAmount(MenuEntry *entry)
	{
		u32 u0 = AutoRegion(0x2D0C84, 0x2D0C8C, 0x2D0C64);
		
		if(entry->WasJustActivated())
		{
			entry->Name() = Color::LimeGreen << "Extend Mushroom Limit";
			Process::Write32(u0, 0xE3A0000F);
		}
        
        if (!entry->IsActivated())
		{
			entry->Name() = "Extend Mushroom Limit";
			Process::Write32(u0, 0xEBFFF2C1);
        }
    }
	
	void	dropStarAmount(MenuEntry *entry)
	{
		u32 u0 = AutoRegion(0x2CECE8, 0x2CECF0, 0x2CECC8);
		
		if(entry->WasJustActivated())
		{
			entry->Name() = Color::LimeGreen << "Drop Star Amount Modifier";
			Process::Write32(u0, 0xE3A0000F);
        }
        
        if (!entry->IsActivated())
		{
            entry->Name() = "Drop Star Amount Modifier";
            Process::Write32(u0, 0xEBFFFADE);
        }
    }
	
	void	bombAmount(MenuEntry *entry)
	{
		u32 u0 = AutoRegion(0x2D08BC, 0x2D08C4, 0x2D089C);
		
		if(entry->WasJustActivated())
		{
			entry->Name() = Color::LimeGreen << "Bob-Omb Amount Modifier";
			Process::Write32(u0, 0xE3A0000F);
        }
        
        if (!entry->IsActivated())
		{
			entry->Name() = "Bob-Omb Amount Modifier";
			Process::Write32(u0, 0xEBFFF3B3);
		}
	}
	
	void	fireballAmount(MenuEntry *entry)
	{
		u32 u0 = AutoRegion(0x2D0AA8, 0x2D0AB0, 0x2D0A88);
		
		if(entry->WasJustActivated())
		{
			entry->Name() = Color::LimeGreen << "Fireball Amount Modifier";
			Process::Write32(u0, 0xE3A0000F);
        }
        
        if (!entry->IsActivated())
		{
			entry->Name() = "Fireball Amount Modifier";
	...
	void	pluginInfo(MenuEntry *entry)
	{
		std::vector<std::string>
		choice = {"Close"};
		Keyboard KB("- Build Information -\n\n" << Color::Yellow << "- Creator: " << Color::White << "SFC@hacker@\n\n" << Color::Yellow << "- Version: " << Color::White << "3.0.3\n\n" << Color::Yellow << "- Last Compiled: " << Color::White << "14/04/2024 13:40\n\n" << Color::Yellow << "- Codes: " << Color::White << "169\n\n" << Color::Yellow << "- Next Update: " << Color::Red << "Missing!", choice);
		KB.Open();
	}
